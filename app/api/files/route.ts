...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // One-line per-request log so deployment-only failures show up
    // searchably in CloudWatch.  Format mirrors the per-request log in
    // `middleware.ts` so operators can grep both.
    console.log(
      `[Files Proxy] op=${fileOp ?? '<none>'} user=${userId} ` +
        `upstream=${upstreamUrl.pathname} status=${response.status} ` +
        `upstream_ms=${fetchMs} total_ms=${Date.now() - reqStart}`,
    );
    ...
    if (!response.ok) {
      ...
      console.warn(
        `[Files Proxy] upstream non-OK: status=${response.status} ` +
