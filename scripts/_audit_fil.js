...
const data = JSON.parse(fs.readFileSync(path.join(__dirname, '..', 'messages', 'fil.json'), 'utf8'));
...
for (const [k, budget, policy] of keys) {
  const v = get(data, k);
  if (v === undefined) {
    result.push({ key: k, value: '<MISSING>', char_count: 0, budget, policy, ratio: 0 });
    continue;
  }
  const cc = [...v].length;
  result.push({ key: k, value: v, char_count: cc, budget, policy, ratio: Number((cc/budget).toFixed(2)) });
}
