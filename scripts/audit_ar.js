...
const data = JSON.parse(fs.readFileSync(path.join(__dirname, '..', 'messages', 'ar.json'), 'utf8'));
...
for (const [key, budget, wrap] of keys) {
  ...
  const ratio = (count / budget).toFixed(2);
  let flag = "  ";
  if (wrap === 'must_fit' && count > budget) flag = "!!";
  else if (wrap === 'truncate' && count > budget) flag = "!T";
  else if (count > budget) flag = "~~";
  console.log(flag + " " + key + " | " + count + "/" + budget + " (" + ratio + "x) [" + wrap + "] | " + val);
}
