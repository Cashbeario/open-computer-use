...
const data = JSON.parse(fs.readFileSync(path.join(__dirname, '..', 'messages', 'es.json'), 'utf8'));
...
for (const [k, budget, policy] of keys) {
  const v = get(data, k);
  if (v == null) {
    out.push({key: k, value: '<MISSING>', len: 0, budget, policy, ratio: 0});
  } else {
    const len = [...v].length;
    const ratio = +(len/budget).toFixed(2);
    out.push({key: k, value: v, len, budget, policy, ratio});
  }
}
