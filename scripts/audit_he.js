...
const data = JSON.parse(fs.readFileSync(path.join(__dirname, '..', 'messages', 'he.json'), 'utf8'));
...
for (const [key, budget, policy] of keys) {
  ...
  const len = val == null ? -1 : [...val].length;
  out.push({key, value: val, len, budget, policy, ratio: len > 0 ? +(len/budget).toFixed(2) : 0, over: len > budget});
}
