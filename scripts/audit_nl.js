...
const data = JSON.parse(fs.readFileSync(path.join(__dirname, '..', 'messages', 'nl.json'), 'utf8'));
...
for (const [k, budget, policy] of keys) {
  ...
  const flag = len > budget ? '!' : ' ';
  console.log(flag + ' ' + k + ' | ' + len + '/' + budget + ' (' + ratio.toFixed(2) + 'x) [' + policy + ']');
  if (len > budget) console.log('    VALUE: ' + JSON.stringify(v));
}
