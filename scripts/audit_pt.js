...
const data = JSON.parse(fs.readFileSync(path.join(__dirname, '..', 'messages', 'pt.json'), 'utf8'));
...
for (const [key, budget, policy] of keys) {
  ...
  const flag = (policy !== 'wrap_unlimited' && ratio > 1.0) ? ' [OVER]' : (ratio > 1.0 ? ' [over-soft]' : '');
  console.log(key + ' | ' + len + '/' + budget + ' (x' + ratio.toFixed(2) + ') [' + policy + ']' + flag);
  console.log('  -> ' + val);
}
