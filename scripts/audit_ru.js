...
const data = JSON.parse(fs.readFileSync(path.join(__dirname, '..', 'messages', 'ru.json'), 'utf8'));
...
const out = [];
for (const [k, budget, policy] of keys) {
  const v = get(data, k);
  if (v == null) {
    out.push({key: k, value: '(MISSING)', char_count: 0, budget, policy, ratio: 0});
    continue;
  }
  const cc = String(v).length;
  const ratio = cc / budget;
  out.push({key: k, value: v, char_count: cc, budget, policy, ratio: +ratio.toFixed(2)});
}
