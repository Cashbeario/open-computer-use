...
for (const [k, budget, policy] of keys) {
  ...
  if (v === undefined) {
    console.log(k + '\t<MISSING>\tbudget=' + budget);
  } else {
    const count = [...String(v)].length;
    const stripped = String(v).replace(/\{[^}]+\}/g, '');
    const strippedCount = [...stripped].length;
    const over = count > budget;
    const ratio = (count / budget).toFixed(2);
    console.log(k + '\tcount=' + count + '\tstripped=' + strippedCount + '\tbudget=' + budget + '\tpolicy=' + policy + (over ? '\tOVER ratio=' + ratio : '') + '\t| ' + v);
  }
}
